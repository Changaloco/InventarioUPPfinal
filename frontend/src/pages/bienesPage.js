...
function Bienes() {
  ...
  const [imageFactura, setImageFactura] = useState([]);
  ...
  //*Esta funcion esta encargada de la gestion de las imagenes.
  const uploadImage = (e) => {
  ...
  const uploadImageFactura = (e) => {
    setImageFactura(e.target.files[0]);
    console.log(imageFactura);
  };
  //* Funcion de insercion de bienes #tambien controla la insercion de la imagen en el formulario
  const insertBien = async () => {
    bienSelected.montoDepreciacion = 350;
    console.log(bienSelected.montoDepreciacion);
    let formdata = new FormData();
    formdata.append("image", image2);
    formdata.append("image", imageFactura);
    ...
    formdata.append(
      "numeroInventarioAnterior",
      bienSelected.numeroInventarioAnterior
    );
    formdata.append(
      "numeroInventarioArmonizado",
      bienSelected.numeroInventarioArmonizado
    );
    formdata.append(
      "clasificacionAdicional",
      bienSelected.clasificacionAdicional
    );
    ...
    formdata.append(
      "tratamientoAdministrativo",
      bienSelected.tratamientoAdministrativo
    );
    ...
    formdata.append(
      "id_clasificacionConac",
      bienSelected.id_clasificacionConac
    );
    ...
    formdata.append(
      "id_catalogoDepreciacion",
      bienSelected.id_catalogoDepreciacion
    );
    await axios
      .post("http://localhost:4000/api/bienes", formdata)
      .then(
        (response) => setBienes(bienes.concat(response.data)),
  ...
  const deleteBien = async () => {
    await axios.delete("bienes/" + bienSelected.id_Bien).then((response) => {
      setBienes(
        bienes.filter((bienes) => bienes.id_Bien !== bienSelected.id_Bien)
      );
  ...
  return (
    <>
            ...
            {bienes.map((bienes) => (
              <tr key={bienes.id_Bien}>
                ...
                <td>
                  <Button
                    variant="success"
                    onClick={() => selectBien(bienes, "Editar")}
                  >
                    Editar
                  </Button>
                  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                  <Button
                    variant="danger"
                    onClick={() => selectBien(bienes, "Eliminar")}
                  >
                    Eliminar
                  </Button>
                </td>
      ...
      <Modal show={modalInsert} onHide={OpenCloseModalInsert}>
        ...
        <Modal.Body>
          <Form>
            ...
            <Form.Group>
              <Form.Label>Fotografia 2 del Bien</Form.Label>
              <Form.File name="image2" onChange={uploadImage2} />
            </Form.Group>
            <Form.Group>
              <Form.Label>Fotografia de la factura del bien</Form.Label>
              <Form.File name="imagefactura" onChange={uploadImageFactura} />
            </Form.Group>
            <Form.Group>
      ...
      <Modal show={modalDelete} onHide={() => OpenCloseModalDelete()}>
        ...
        <Modal.Body>Esta seguro de que desea eliminar este bien ?</Modal.Body>
        <Modal.Footer>
