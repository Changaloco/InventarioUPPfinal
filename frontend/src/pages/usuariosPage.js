...
function Usuarios() {
  ...
  const editUser = async () => {
    await axios
      .put(
        "http://localhost:4000/api/usuarios/" + userSelected.id_Usuario,
        userSelected
      )
      .then((response) => {
        var dataNueva = users;
  ...
  const insertUser = async () => {
    ...
    formData.append(
      "perfilAcademicoUsuario",
      userSelected.perfilAcademicoUsuario
    );
    formData.append("puestoUsuario", userSelected.puestoUsuario);
    formData.append(
      "estatusLaboralUsuario",
      userSelected.estatusLaboralUsuario
    );
    formData.append("id_Departamento", userSelected.id_Departamento);
    formData.append("id_Area", userSelected.id_Area);
    console.log(formData);
    await axios
      .post("http://localhost:4000/api/usuarios/register", formData)
      .then(
        (response) => setUsuarios(users.concat(response.data)),
  ...
  const deleteUser = async () => {
    await axios
      .delete("http://localhost:4000/api/usuarios/" + userSelected.id_Usuario)
      .then((response) => {
        setUsuarios(
          users.filter((users) => users.id_Usuario !== userSelected.id_Usuario)
        );
        setUserSelected(null);
            ...
            <tbody>
              {users.map((usuario) => (
                <tr key={usuario.id_Usuario}>
                  ...
                  <td>
                    <Button
                      onClick={() => selectUsuario(usuario, "Editar")}
                      variant="success"
                    >
                      Editar
                    </Button>
                    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                    <Button
                      onClick={() => selectUsuario(usuario, "Eliminar")}
                      variant="danger"
                    >
                      Eliminar
                    </Button>
          ...
          <Form>
            ...
            <Form.Group controlId="exampleForm.ControlInput1">
              ...
              <Form.Control
                ...
                value={userSelected && userSelected.apellidoPUsuario}
                onChange={handleChange}
              />
            </Form.Group>
            <Form.Group controlId="exampleForm.ControlInput1">
              ...
              <Form.Control
                ...
                value={userSelected && userSelected.apellidoMUsuario}
                onChange={handleChange}
              />
            </Form.Group>
            ...
            <Form.Group controlId="formBasicEmail">
              ...
              <Form.Control
                type="email"
                name="correoUsuario"
                value={userSelected && userSelected.correoUsuario}
                onChange={handleChange}
              />
              <Form.Text className="text-muted">
                Usa tu correo de la universidad!
            ...
            <Form.Group controlId="formBasicPassword">
              ...
              <Form.Control
                type="password"
                placeholder="Password"
                value={userSelected && userSelected.passwordUsuario}
                name="passwordUsuario"
                onChange={handleChange}
              />
            </Form.Group>
            <Form.Group controlId="exampleForm.ControlSelect1">
              ...
              <Form.Control
                as="select"
                name="tipoUsuario"
                value={userSelected && userSelected.tipoUsuario}
                onChange={handleChange}
              >
                <option>ADMINISTRADOR</option>
                <option>INVITADO</option>
            ...
            <Form.Group controlId="exampleForm.ControlSelect1">
              ...
              <Form.Control
                as="select"
                name="perfilAcademicoUsuario"
                value={userSelected && userSelected.perfilAcademicoUsuario}
                onChange={handleChange}
              >
                <option>Doctorado en Ciencias en Biotecnología</option>
                <option>Licenciatura en Ciencias de la Comunicación</option>
            ...
            <Form.Group controlId="exampleForm.ControlSelect1">
              ...
              <Form.Control
                as="select"
                name="puestoUsuario"
                value={userSelected && userSelected.puestoUsuario}
                onChange={handleChange}
              >
                <option>PROFESOR INVESTIGADOR TITULAR A</option>
                <option>PROFESOR INVESTIGADOR TITULAR B</option>
            ...
            <Form.Group controlId="exampleForm.ControlSelect1">
              ...
              <Form.Control
                name="estatusLaboralUsuario"
                value={userSelected && userSelected.estatusLaboralUsuario}
                onChange={handleChange}
                as="select"
              >
                <option>Activo</option>
            ...
            <Form.Group controlId="exampleForm.ControlSelect1">
              ...
              <Form.Control
                name="id_Departamento"
                onChange={handleChange}
                value={userSelected && userSelected.id_Departamento}
                as="select"
              >
                {departamentos.map((departamentos) => (
                  <option value={departamentos.id_Departamento}>
            ...
            <Form.Group controlId="exampleForm.ControlSelect1">
              ...
              <Form.Control
                name="id_Area"
                onChange={handleChange}
                value={userSelected && userSelected.id_Area}
                as="select"
              >
                {areas.map((areas) => (
                  <option value={areas.id_Area}>{areas.nombreArea}</option>
