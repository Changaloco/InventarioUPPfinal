...
function BienIndividual(props) {
  ...
  const insertComentario = async () => {
    await axios
      ...
      .then(
        (response) => setComentarios(comentarios.concat(response.data)),
  ...
  const fetchComentarios = async () => {
    await axios.get("comentarios/search/" + id).then((response) => {
  ...
  const deleteComentario = async () => {
    await axios
      .delete("comentarios/" + comentarioSelect.id_Comentario)
      .then((response) => {
        setComentarios(
          comentarios.filter(
            (comentarios) =>
              comentarios.id_Comentario !== comentarioSelect.id_Comentario
          )
        );
        setComentarioSelect(null);
        ...
        <Container>
          <Row>
            <Col>
              <Carousel>
                ...
                <Carousel.Item>
                  <img
                    className="d-block w-100"
                    src={"http://localhost:4000/" + bien.fotografiaBien2}
                    alt="Second slide"
                  />
                </Carousel.Item>
                <Carousel.Item>
                  <img
                    className="d-block w-100"
                    src={"http://localhost:4000/" + bien.fotografiaFactura}
                    alt="Third slide"
                  />
                </Carousel.Item>
              </Carousel>
            ...
            <Col>
              ...
              <h4>
                Numero Inventario Armonizado: {bien.numeroInventarioArmonizado}
              </h4>
              <h4>
                Numero Inventario Anterior: {bien.numeroInventarioAnterior}
              </h4>
              ...
              <Button
                href={`http://localhost:4000/${bien.etiquetaBien}`}
                variant="primary"
                download
              >
                Descargar Etiqueta
        ...
        <Modal.Body>
          <Form>
            <Form.Group>
              <Form.Label>Titulo del comentario</Form.Label>
              <Form.Control
                value={comentarioSelect && comentarioSelect.titulo}
                name="titulo"
            ...
            <Form.Group>
              <Form.Label>Comentario</Form.Label>
              <Form.Control
                value={comentarioSelect && comentarioSelect.comentario}
                name="comentario"
                onChange={handleChange}
              />
            </Form.Group>
            <Form.Group>
              <Form.Label>Estado</Form.Label>
              <Form.Control
                value={comentarioSelect && comentarioSelect.estado}
                name="estado"
                onChange={handleChange}
              />
            </Form.Group>
